...
from city_configs import CompanyConfig, DEFAULT_COMPANIES
...
class EmailDispatchModule:
    ...
    def _send_emails(self, event: ft.ControlEvent) -> None:
        """Processa e envia emails pelo provedor selecionado."""
        try:
            ...
            for payload in payloads:
                try:
                    ...
                    errors.append(f"{payload.recipient}: {send_exc}")

            if errors:
                summary = "; ".join(errors[:3])
                suffix = "..." if len(errors) > 3 else ""
                self.status_text.value = f"⚠️ {sent_count}/{recipient_count} email(s) enviados. Falhas: {summary}{suffix}"
                self.status_text.color = "#F27D16"
            else:
...
def build_email_dispatch_module(
    ...
    return EmailDispatchModule(page=page, on_back=on_back, config=config, ic_func=ic_func)
