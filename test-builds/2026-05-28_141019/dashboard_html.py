...
def _mini_insight(emoji: str, cor: str, texto: str) -> str:
    ...
    Observação: o campo `emoji` é ignorado (removido do uso) para não depender
    de caracteres emoji. O ícone é escolhido com base em `cor` e, quando possível,
    no conteúdo textual.
    """
    cor_map = {
        'green':  ('rgba(46,125,50,0.08)',  '#2E7D32', 'rgba(46,125,50,0.35)', 'check_circle'),
        'red':    ('rgba(198,40,40,0.08)',   '#C62828', 'rgba(198,40,40,0.35)', 'error'),
        'orange': ('rgba(230,115,0,0.08)',   '#E65100', 'rgba(230,115,0,0.35)', 'warning'),
        'blue':   ('rgba(21,101,192,0.08)',  '#1565C0', 'rgba(21,101,192,0.35)', 'info'),
        '':       ('rgba(100,116,139,0.07)', '#475569', 'rgba(100,116,139,0.25)', 'lightbulb'),
    }

    # Normaliza e escolhe ícone por cor
    cor_norm = str(cor or '').strip().lower()
    bg, txt_cor, border, icon_por_cor = cor_map.get(cor_norm, cor_map[''])

    # Refinamento por contexto (sem usar emoji literal)
    t = str(texto or '').upper()
    icon = icon_por_cor
    if any(k in t for k in ['CRÍTICA', 'CRITICA', 'RISCO', 'ERRO', 'ALERTA', 'PROTESTO', 'JUDICI', 'CORTE', 'BLOQUEIO']):
        icon = 'error'
    elif 'ZERO' in t or 'CONSUMO ZERO' in t or 'SEM ARRECADAÇÃO' in t or 'SEM ARRECADACAO' in t:
        icon = 'visibility_off'
    elif any(k in t for k in ['MELHOR', 'LIDER', 'MAIOR', 'EXCELENTE', 'ALTA']):
        icon = 'trending_up'
    elif any(k in t for k in ['PIOR', 'MENOR', 'BAIXA', 'QUEDA']):
        icon = 'trending_down'
    elif any(k in t for k in ['MÉDIA', 'MEDIA']):
    ...
    return (
        ...
        f'<span class="material-symbols-rounded" style="font-size:1.1rem;flex-shrink:0;margin-top:1px">{icon}</span>'
...
def _insights_cards_html(items: list) -> str:
  ...
  if not items:
    return (
      ...
      '<div class="insight-title"><span class="material-symbols-rounded" style="font-size:14px;vertical-align:-2px;margin-right:4px;">info</span>Sem insights suficientes</div>'
  ...
  for it in items:
    ...
    cards.append(
      ...
      f'<div class="insight-title"><span class="material-symbols-rounded" style="font-size:14px;vertical-align:-2px;margin-right:4px;">{icon}</span>{title}</div>'
...
def gerar_pn_html(dados: dict, caminho_saida: str, _return_parts: bool = False) -> str:
    ...
    nome_empresa = "Itapoá Saneamento"
    try:
        from city_configs import get_city_config
        cfg = get_city_config(cidade)
        if cfg:
            nome_empresa = cfg.name
    except Exception:
        pass
    ...
    # ── Identidade visual (logo embutido em base64) ──
    logo_data_uri = ""
    logo_candidates = [
        "Itapoa_branca.png",
        "itapoa_informa.png",
    ...
    try:
        from logos_base64 import get_logo
        logo_data_uri = get_logo(cidade)
    except Exception:
        pass
    ...
    inicial_nome = "".join([w[0] for w in nome_empresa.split() if w])[:2].upper() if nome_empresa else "IS"

    logo_html = (
        f'<img class="brand-logo" src="{logo_data_uri}" alt="Logo {nome_empresa}" style="max-width:64px;max-height:64px">'
        if logo_data_uri else
        f'<div class="brand-logo-fallback" title="{nome_empresa}">{inicial_nome}</div>'
    )
    # Fallback seguro se logo_html ficar vazio
    if not logo_html or logo_html.isspace():
        logo_html = f'<div class="brand-logo-fallback">{inicial_nome}</div>'
    ...
    if meses_ordenados:
        ...
        for m in meses_ordenados:
            ...
            for d in sorted(m["dias"], key=lambda x: x["dt"]):
                ...
                """
                dias_data.append({
                    "dt_str": d['dt'].strftime('%d/%m/%Y'),
                    "fd": d["fd"],
                    "fat_mes_ac": fat_mes_ac,
                    "dif_fat_dia": dif_fat_dia,
                    "ad": d["ad"],
                    "arr_mes_ac": arr_mes_ac,
                    "dif_arr_dia": dif_arr_dia
                })
            tabela_mensal_data.append({
                "key": m['key'],
                "label": m['label'],
                "fm": fm,
                "fat_acum": fat_acum,
                "dif_fat_pct": dif_fat_pct,
                "am": am,
                "arr_acum": arr_acum,
                "dif_arr_pct": dif_arr_pct,
                "ref_prev": _ref_prev,
                "ref_next": _ref_next,
                "dias": dias_data
            })
    ...
    if isinstance(serie_91, list) and serie_91:
        ...
        for s in serie_91_ord:
            ...
            """
            tabela_8091_data.append({
                "periodo": s.get('periodo', '-'),
                "fat_agua": fat_agua,
                "fat_agua_acum": fat_agua_acum,
                "vol_fat_agua": vol_fat_agua,
                "eco_agua_fat": int(round(eco_agua_fat)),
                "fat_esgoto": fat_esgoto,
                "fat_esgoto_acum": fat_esgoto_acum,
                "eco_esc_fat": int(round(eco_esc_fat)),
                "dias_lidos": dias_lidos
            })
    else:
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .insight-card {{ border:1px solid var(--line); border-left:4px solid #93A7C3; border-radius:10px; padding:12px 13px; background:#F8FAFE; display:flex; flex-direction:column; gap:4px; }}
    .insight-title {{ font-size:12px; font-weight:800; color:#153354; margin-bottom:2px; text-transform:uppercase; letter-spacing:0.3px; display:flex; align-items:center; gap:5px; }}
    .insight-title .material-symbols-rounded {{ font-size:16px; flex-shrink:0; }}
    .insight-text {{ font-size:13px; color:#31465E; line-height:1.45; padding-left:21px; }}
    .insight-card.tone-positive {{ background:linear-gradient(135deg,#F1FBF6 0%,#E8F8F0 100%); border-left-color:#1E8E5A; }}
    .insight-card.tone-positive .insight-title {{ color:#0D6A41; }}
    .insight-card.tone-positive .insight-title .material-symbols-rounded {{ color:#1E8E5A; }}
    .insight-card.tone-warning {{ background:linear-gradient(135deg,#FFF7EE 0%,#FFF1E0 100%); border-left-color:#D07A00; }}
    .insight-card.tone-warning .insight-title {{ color:#955700; }}
    .insight-card.tone-warning .insight-title .material-symbols-rounded {{ color:#D07A00; }}
    .insight-card.tone-info {{ background:linear-gradient(135deg,#EFF5FF 0%,#E5EFFF 100%); border-left-color:#2C6AC5; }}
    .insight-card.tone-info .insight-title {{ color:#1E4F97; }}
    .insight-card.tone-info .insight-title .material-symbols-rounded {{ color:#2C6AC5; }}
    .insight-card.tone-neutral {{ background:linear-gradient(135deg,#F5F7FB 0%,#EEF2F8 100%); border-left-color:#8193AA; }}
    .insight-card.tone-neutral .insight-title {{ color:#3D5674; }}
    .insight-card.tone-neutral .insight-title .material-symbols-rounded {{ color:#8193AA; }}
  ...
  <div class="container">
        ...
        <div>
          <div class="brand-row">
            ...
            <div>
              ...
              <p>{nome_empresa} | Unidade {cidade} | Consolidado mensal em milhares (R$ k)</p>
            </div>
          </div>
        </div>
        <div class="meta">
    ...
    <div class="footnote">
      Relatório Gerencial PN - Mathools 1.0  {nome_empresa}<br>
...
<body>
  ...
  (() => {{
    ...
    if _return_parts:
        ...
        # Parser simples para prefixar #panel-pn nas regras CSS
        clean_css = _re.sub(r'/\*.*?\*/', '', _css_raw, flags=_re.DOTALL)
        tokens = _re.split(r'([{}])', clean_css)
        scoped_css = ""
        i = 0
        while i < len(tokens):
            text = tokens[i]
            if not text.strip() and i + 1 < len(tokens) and tokens[i+1] == '}':
                # Fechamento de @media
                scoped_css += "}\n"
                i += 2
                continue
            ...
            if i + 1 < len(tokens) and tokens[i+1] == '{':
                ...
                if sel_text.startswith('@media'):
                    ...
                    for s in sels:
                        if not s: continue
                        if s == ':root' or s.lower() == 'body':
                            new_sels.append('#panel-pn')
                        elif s.startswith('@'):
                            new_sels.append(s)
                        else:
                            new_sels.append(f'#panel-pn {s}')
                    scoped_css += ", ".join(new_sels) + " {\n"
                i += 2
...
f'<span style="background:rgba(255,255,255,0.15);border:1px solid rgba(255,255,255,0.25);'
    ...
    # Insights HTML 8091
    _ins91_html = ''
    _icon91_map = {'green': 'check_circle', 'red': 'error', 'orange': 'warning', '': 'lightbulb'}
    for it in ins91:
        _icon91 = _icon91_map.get(it.get('cor', ''), 'lightbulb')
        _ins91_html += (
            f'<div class="insight-item {it["cor"]}">'
            f'<span class="insight-emoji material-symbols-rounded">{_icon91}</span>'
...
<meta charset="UTF-8"/>
<meta name="viewport" content="width=device-width, initial-scale=1.0"/>
<title>Dashboard Financeiro  {cidade} | {nome_empresa}</title>
<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Material+Symbols+Rounded:opsz,wght,FILL,GRAD@20..48,100..700,0..1,-50..200" />
...
<style>
  ...
  * {{ margin:0; padding:0; box-sizing:border-box; }}
  ...
  .insight-emoji {{ font-size: 1.2rem; line-height: 1; flex-shrink: 0; }}
  .insight-item.orange .insight-emoji {{ color: var(--orange); }}
  .insight-item.green  .insight-emoji {{ color: var(--green); }}
  .insight-item.red    .insight-emoji {{ color: var(--red); }}
  .insight-item:not(.orange):not(.green):not(.red) .insight-emoji {{ color: var(--navy); }}
...
<div class="header">
  <div class="header-top">
    <div class="header-left">
      ...
      <p>{nome_empresa} &nbsp;|&nbsp; {macros_badges}</p>
    </div>
...
<div class="footer">
  Gerado automaticamente pelo <strong>Mathools 1.0  {nome_empresa}</strong>
...
function initInsights8117() {{
  ...
  items.forEach((it, i) => {{
    ...
    const iconMap = {{ green: 'check_circle', red: 'error', orange: 'warning', '': 'lightbulb' }};
    const icon = iconMap[it.cor] || 'lightbulb';
    div.innerHTML = `<span class="insight-emoji material-symbols-rounded">${{icon}}</span><div class="insight-text">${{it.texto}}</div>`;
