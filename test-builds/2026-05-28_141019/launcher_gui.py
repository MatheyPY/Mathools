...
try:
    from updater import check_update_async, apply_update, CURRENT_VERSION as APP_VERSION
    _HAS_UPDATER = True
except ImportError:
    _HAS_UPDATER = False
...
def keep_system_awake(awake: bool = True):
    """Impede que o Windows entre em suspensão/hibernação durante envios longos."""
    import sys
    if sys.platform == "win32":
        try:
            import ctypes
            # Constantes do SetThreadExecutionState
            ES_CONTINUOUS = 0x80000000
            ES_SYSTEM_REQUIRED = 0x00000001
            ES_DISPLAY_REQUIRED = 0x00000002
            if awake:
                # Impede suspensão do sistema e desligamento da tela
                ctypes.windll.kernel32.SetThreadExecutionState(ES_CONTINUOUS | ES_SYSTEM_REQUIRED | ES_DISPLAY_REQUIRED)
            else:
                # Restaura comportamento normal (permite suspensão)
...
try:
    from auth_system import get_headers as _auth_get_headers
    def get_headers():
...
# ─────────────────────────────────────────────────────────────────────────────
# MÓDULOS WATERFY - Multi-cidade histogramas
# ─────────────────────────────────────────────────────────────────────────────
try:
    from waterfy_engine import CIDADES as DEFAULT_COMPANIES, TIPOS as TIPOS_RELATORIO, WaterfyHistogramasManager
    _HAS_WATERFY_MODULES = True
except ImportError as e:
    _HAS_WATERFY_MODULES = False
    DEFAULT_COMPANIES = {}
    def get_city_config(name):
        return None
    class WaterfyHistogramasManager:
        pass
    TIPOS_RELATORIO = {}
...
def build_dashboard(num_operadores: int = 0, lq=None, page: ft.Page = None, topbar_title_ref=None):
    ...
    def _on_city_change(e, title_ref):
        from city_configs import get_city_config
        selected_city = e.control.value
        config = get_city_config(selected_city)
        if config and title_ref:
            title_ref.value = config.name
        elif title_ref:
            title_ref.value = "Mathools" # Fallback
        try:
            if title_ref:
                title_ref.update()
        except Exception:
    ...
    # ── SEÇÃO DE HISTOGRAMAS (WATERFY) ────────────────────────────────────
    if _HAS_WATERFY_MODULES and DEFAULT_COMPANIES:

        # Checkbox para ativar/desativar histogramas
        chk_incluir_histogramas = ft.Checkbox(
            value=False,
            active_color=NAVY,
            label="Incluir Histogramas Waterfy (baixa TODOS os tipos disponíveis)",
        )

        # Campos de período para histogramas (mes_ini/mes_fim)
        e_hist_mes_ini = ft.TextField(
            label="Mês/Ano Inicial", value=_hoje.strftime("%m/%Y"),
            hint_text="ex: 01/2026",
            prefix_icon=ic("CALENDAR_MONTH"), height=52, border_radius=10,
            content_padding=ft.Padding.symmetric(horizontal=14, vertical=10),
        ...
        row_hist_periodo = ft.Row(
            [e_hist_mes_ini, ft.Container(width=12), e_hist_mes_fim, ft.Container(expand=True)],
        )

        col_histogramas = ft.Column([
            ft.Row([
                ft.Icon(ic("SHOW_CHART"), size=14, color=NAVY),
                ft.Text("Histogramas Waterfy", size=13, weight=ft.FontWeight.BOLD, color=TEXT_MAIN),
            ], spacing=6),
            ft.Container(height=8),
            ft.Row([chk_incluir_histogramas, ft.Container(expand=True)]),
            ft.Container(height=8),
            ft.Text("Período (mes/ano)", size=11, color=TEXT_MAIN, weight=ft.FontWeight.W_500),
            row_hist_periodo,
        ], spacing=0)
    else:
        col_histogramas = ft.Column([
            ft.Text(
                "Módulo de histogramas não disponível. Verifique city_configs.py e waterfy_*.py",
    ...
    def on_gerar(e):
        ...
        # Verificar se histogramas estão ativados
        histogramas_ativados = False
        if _HAS_WATERFY_MODULES:
            try:
                histogramas_ativados = chk_incluir_histogramas.value
            except:
                pass
        ...
        # Validar: precisa ter macros OU histogramas
        if not macros_sel and not histogramas_ativados:
            status_txt.value = "Selecione ao menos uma Macro ou ative Histogramas."
            status_txt.color = RED; status_txt.update()
            if lq is not None:
                try: lq.put("Validacao: selecione ao menos uma macro ou ative histogramas.\n")
                except Exception: pass
            return

        # Validar histogramas se ativados
        if histogramas_ativados:
            cidade_hist = None
            try:
                cidade_hist = ddl_city_global.value if hasattr(ddl_city_global, 'value') else None
            except:
                pass

            if not cidade_hist:
                status_txt.value = "Selecione uma cidade para histogramas."
                status_txt.color = RED; status_txt.update()
                if lq is not None:
                    try: lq.put("Validacao: selecione uma cidade para histogramas.\n")
                    except Exception: pass
                return

            # Validar período
            mi_str = e_hist_mes_ini.value.strip()
            mf_str = e_hist_mes_fim.value.strip()
            if not mi_str or len(mi_str) != 7 or not mf_str or len(mf_str) != 7:
                status_txt.value = "Preencha Mês/Ano (MM/AAAA) para histogramas."
                status_txt.color = RED; status_txt.update()
                if lq is not None:
                    try: lq.put("Validacao: preencha mes/ano em formato MM/AAAA para histogramas.\n")
                    except Exception: pass
                return

            try:
                data_ini_hist = datetime.strptime(f"01/{mi_str}", "%d/%m/%Y")
                data_fim_hist = datetime.strptime(f"01/{mf_str}", "%d/%m/%Y")
                if data_ini_hist > data_fim_hist:
                    status_txt.value = "Mês/Ano Inicial não pode ser maior que o Final."
                    status_txt.color = RED; status_txt.update()
                    if lq is not None:
                        try: lq.put("Validacao: mes inicial nao pode ser maior que o final.\n")
                        except Exception: pass
                    return
            except Exception:
                status_txt.value = "Formato de data inválido."
                status_txt.color = RED; status_txt.update()
                return

            # Iniciar download de histogramas em thread
            status_txt.value = "Iniciando download de histogramas..."
            status_txt.color = NAVY
            status_txt.update()

            def _download_histogramas_thread():
                try:
                    from datetime import datetime
                    from pathlib import Path
                    base = _get_base()
                    pasta_brutos = Path(base) / "Histogramas_Waterfy" / cidade_hist / "Brutos"
                    pasta_brutos.mkdir(parents=True, exist_ok=True)
                    pasta_limpos = Path(base) / "Histogramas_Waterfy" / cidade_hist / "Limpos"

                    manager = WaterfyHistogramasManager(cidade_hist, usuario, senha)

                    if not macros_sel:
                        status_txt.value = "Realizando login..."
                        status_txt.color = NAVY
                        try: status_txt.update()
                        except: pass

                    if lq is not None:
                        try: lq.put(f"[Histogramas] Login em {cidade_hist}...\n")
                        except: pass

                    if not manager.fazer_login():
                        if not macros_sel:
                            status_txt.value = "Falha ao fazer login na cidade selecionada."
                            status_txt.color = RED
                            try: status_txt.update()
                            except: pass
                        if lq is not None:
                            try: lq.put(f"[Histogramas] Login falhou para {cidade_hist}.\n")
                            except: pass
                        return

                    # Baixar TODOS os tipos de relatório
                    tipos_disponiveis = list(TIPOS_RELATORIO.keys()) if TIPOS_RELATORIO else []
                    total_ok = 0
                    total_erro = 0
                    ...
                    meses = []
                    cur = data_ini_hist
                    while cur <= data_fim_hist:
                        meses.append((cur.month, cur.year))
                        nm = cur.month + 1
                        na = cur.year + (1 if nm > 12 else 0)
                        nm = 1 if nm > 12 else nm
                        cur = datetime(na, nm, 1)

                    # Itera sobre TODOS os tipos disponíveis
                    for tipo_nome in tipos_disponiveis:
                        for mes, ano in meses:
                            try:
                                msg = f"Baixando {tipo_nome} {mes:02d}/{ano}..."
                                if not macros_sel:
                                ...
                                if lq is not None:
                                    try: lq.put(f"[Histogramas] {msg}\n")
                                    except: pass

                                caminho = manager.baixar_relatorio(
                                    mes=mes, ano=ano, tipo_nome=tipo_nome, pasta_saida=pasta_brutos
                                )
                                if caminho is not None:
                                    total_ok += 1
                                    if lq is not None:
                                        try: lq.put(f"[Histogramas] OK: {caminho.name}\n")
                                        except: pass
                            except Exception as ex:
                                total_erro += 1
                                if lq is not None:
                                    try: lq.put(f"[Histogramas] ERRO {tipo_nome} {mes:02d}/{ano}: {str(ex)[:50]}\n")
                                    except: pass

                    # Fase 2: Consolidar Arquivos
                    if not macros_sel:
                        status_txt.value = "Limpando e consolidando CSVs..."
                        status_txt.color = NAVY
                        try: status_txt.update()
                        except: pass
                    if lq is not None:
                        try: lq.put(f"[Histogramas] Processando e consolidando arquivos...\n")
                        except: pass

                    try:
                        manager.consolidar(pasta_brutos, pasta_limpos)
                        msg_final = f"Histogramas concluídos! {total_ok} arquivos baixados e consolidados."
                        if not macros_sel:
                            status_txt.color = GREEN
                    except Exception as ex:
                        msg_final = f"Downloads OK, mas erro na consolidação: {ex}"
                        if not macros_sel:
                            status_txt.color = ORANGE

                    # Resultado final
                    if not macros_sel:
                        status_txt.value = msg_final
                        try: status_txt.update()
                        except: pass
                    if lq is not None:
                        try: lq.put(f"[Histogramas] {msg_final}\n")
                        except: pass

                except Exception as ex:
                    if not macros_sel:
                        status_txt.value = f"Erro ao baixar histogramas: {ex}"
                        status_txt.color = RED
                        try: status_txt.update()
                        except: pass
                    if lq is not None:
                        try: lq.put(f"[Histogramas] ERRO: {str(ex)[:100]}\n")
                        except: pass
                finally:
                    if manager and manager.session:
                        manager.session.close()

            thread_hist = threading.Thread(target=_download_histogramas_thread, daemon=True)
            thread_hist.start()
            if not macros_sel:
                return  # Não continua com macros, encerra aqui
        ...
        # Continuar com fluxo de macros normalmente
        ...
        _usr = usuario
        _pwd = senha
        _cidade_sel = ddl_city_global.value if hasattr(ddl_city_global, 'value') and ddl_city_global.value else "Itapoá"
            ...
            try:
                ...
                if not _macros and not _pn_selecionado:
                    ...
                    if _dados_combo_mapa:
                        _df_nome = datetime.now().strftime('%Y-%m-%d_%Hh%Mm')
                        ...
                        dados_mapa_only = {
                            ...
                            'cidade': _cidade_sel if _cidade_sel else 'ITAPOÁ',
                ...
                if _pval_first is None or (isinstance(_pval_first, tuple) and not _pval_first[0]):
                    ...
                    _df_nome = "-".join(reversed(_pval_first[-1].split('/')))
                elif isinstance(_pval_first, tuple):
                    _df_nome = "-".join(reversed(_pval_first[1].split('/')))  # data_final
                else:
                    _df_nome = "-".join(reversed(str(_pval_first[1]).split('/')))
                ...
                if 'cidade' not in dados:
                    dados['cidade'] = _cidade_sel if _cidade_sel else 'ITAPOÁ'
                ...
                if _dados_combo_mapa and _dados_combo_mapa.get('status') != 'error':
                    try:
                        ...
                        hoje_eq_fmt = _dt_eq.now().strftime('%Y-%m-%d')
    ...
    cidades_lista_global = list(DEFAULT_COMPANIES.keys()) if _HAS_WATERFY_MODULES and DEFAULT_COMPANIES else ["Itapoá"]
    ddl_city_global = ft.Dropdown(
        label="Unidade / Empresa",
        options=[ft.dropdown.Option(c) for c in cidades_lista_global],
        value="Itapoá",
        width=250,
        height=52,
        border_radius=10,
    ...
    ddl_city_global.on_change = lambda e: _on_city_change(e, topbar_title_ref)
    row_city_global = ft.Row([ddl_city_global, ft.Container(expand=True)])
    ...
    def _lc_on_gerar(e):
        ...
        def _worker():
            try:
                ...
                _cidade_local = ddl_city_global.value if hasattr(ddl_city_global, 'value') and ddl_city_global.value else "Itapoá"
                dados = {"periodo": _periodo, "cidade": _cidade_local}
                ...
                saida = os.path.join(_base, "Dashboard_Waterfy",
                                     f"dashboard_local_{_dt.now().strftime('%Y-%m-%d_%Hh%Mm%Ss')}.html")
                os.makedirs(os.path.dirname(saida), exist_ok=True)
    ...
    def _lc_on_gerar_pn(e):
        ...
        def _worker_pn():
            try:
                ...
                _df_nome = "-".join(reversed(_periodo['final'].split('/')))
...
def build_csv_page(page: ft.Page, lq: queue.Queue, ref_csvs=None):
    ...
    e_os    = tfield("Opcional  Planilha de Ordens de Serviço (OS)")
    is_running = {"v": False}
    ...
    cb_aba_incompletos = ft.Checkbox(
        label="Gerar sheet extra detalhando o status cadastral de todos os clientes (Macro 8162)",
        value=False,
    )

    cb_aba_divida_nao = ft.Checkbox(
        label="Gerar sheet com clientes que NÃO têm dívida do inquilino atual (DIVIDA DO INQUILINO ATUAL = NAO)",
        value=False,
    )
    ...
    def run(e):
        ...
        def worker():
            ...
            _arq_os   = e_os.value.strip() if e_os.value else None
            _corte    = corte_val["n"]
            ...
            try:
                ...
                engine.processar_csv(
                    ...
                    arquivo_os=_arq_os,
                    corte_dias=_corte,
                    gerar_aba_incompletos=cb_aba_incompletos.value,
                    gerar_aba_divida_nao=cb_aba_divida_nao.value,
                )
        ...
        if e_os.value: lq.put(f"   Ordens Serviço  : {e_os.value}\n")
    ...
    return ft.Column([
        ...
        card(ft.Column([
            sec_title("Ordens de Serviço  OS (Opcional)", "ASSIGNMENT"),
            ft.Container(height=10),
            ft.Row([e_os, ft.Container(width=8),
                    sbtn("Procurar", "FOLDER", on_click=lambda e: pick(e_os, e))],
                   vertical_alignment=ft.CrossAxisAlignment.CENTER),
        ])),
        # ── Barra de progresso ────────────────────────────────────────────────
        csv_prog_card,
...
def build_send_page(page: ft.Page, lq: queue.Queue, ref_msgs=None):
    ...
    def _registrar_fechamento_wpp():
        page._close_guard = {
            "kind": "whatsapp",
            "title": "Disparo em andamento",
            "message": (
                "Há um disparo de WhatsApp em andamento.\n"
                "Se você sair agora, o sistema vai pedir a parada, salvar o progresso da planilha "
                "e fechar quando isso terminar."
            ),
            "confirm_label": "Salvar e sair",
            "request_stop": lambda: on_abort(None),
            "wait_message": "Salvando o progresso do disparo de WhatsApp antes de fechar...",
        }

    def _limpar_fechamento_wpp():
        guard = getattr(page, "_close_guard", None)
        if isinstance(guard, dict) and guard.get("kind") == "whatsapp":
            page._close_guard = None
    ...
    def start_send(e):
        ...
        def worker():
            ...
            try:
                ...
                _limpar_fechamento_wpp()
                _refresh_btns()
                if getattr(page, "_pending_close_after_tasks", False):
                    page._pending_close_after_tasks = False
                    try:
                        page.window_destroy()
                    except Exception:
                        try:
...
def build_users_page(page: ft.Page):
    ...
    CIDADES_VALIDAS = ["Itapoá", "Gravatal", "Balneário Gaivota", "Sombrio", "Guabiruba"]
    dd_cidade = ft.Dropdown(
        value="Itapoá",
        options=[ft.dropdown.Option(c) for c in CIDADES_VALIDAS],
        width=200, height=44, text_size=13, border_radius=8,
        filled=True, fill_color="#F8FAFD", border_color=BORDER,
        focused_border_color=ORANGE,
        content_padding=ft.Padding.symmetric(horizontal=14, vertical=10),
    ...
    mod_cbs = {
        ...
        for n in ["Análise de CSV","Disparo WhatsApp","Gestão de Negociação","Logística e Rotas","Disparo de E-mails"]
    }
    ...
    def reset_form():
        ...
        dd_cidade.value = "Itapoá"
        try:
            if btn_add[0]:
                btn_add[0].content.controls[-1].value = "Adicionar Usuário"
        except Exception:
            pass
        try:
            page.update()
    ...
    def sync_table():
        ...
        hdr = ft.Container(
            content=ft.Row([
                ...
                ft.Text("CIDADE",   size=11, weight=ft.FontWeight.BOLD, color=TEXT_MUTED, width=120),
        ...
        for u in users:
            ...
            table_col.controls.append(ft.Container(
                content=ft.Row([
                    ...
                    ft.Row([
                        ft.Icon(ic("LOCATION_ON"), size=13, color=TEXT_MUTED),
                        ft.Text(ucidade, size=12, color=TEXT_MUTED),
                    ], spacing=4, width=120),
    ...
    def start_edit(u):
        ...
        perms_set = {
            m.strip() for m in str(perms).split(",")
            if str(m).strip()
        } if perms != "Todos" else set()
        for m, cb in mod_cbs.items():
            cb.value = (perms == "Todos" or m in perms_set)
        dd_cidade.value = u.get("cidade", "Itapoá") or "Itapoá"
        form_title.value = f"Editando: {u.get('username')}"
        try:
            if btn_add[0]:
                btn_add[0].content.controls[-1].value = "Salvar Alterações"
        except Exception:
            pass
        try:
            page.update()
    ...
    def submit(e):
        ...
        uperms = get_perms()
        ucidade = dd_cidade.value or "Itapoá"
        ...
        if editing_state["user_id"] is None:
            ...
            ok, msg = auth.add_user(uname, upwd, urole, uperms, cidade=ucidade)
        else:
            ok, msg = auth.update_user(editing_state["user_id"], uname, upwd, urole, uperms, cidade=ucidade)
    ...
    return ft.Column([
        ...
        card(ft.Column([
            ...
            ft.Row([
                ...
                ft.Container(width=14),
                ft.Column([flabel("Cidade"),          ft.Container(height=4), dd_cidade], spacing=0),
            ], wrap=True, spacing=0),
            ft.Container(height=14),
...
def build_help_page(user=None):
    ...
    _mods_set = {
        m.strip() for m in str(_mods).split(",")
        if str(m).strip()
    } if _mods != "Todos" else set()
    ...
    def _can_view_help_module(mod):
        ...
        return _role == "ADM" or _mods == "Todos" or access_module in _mods_set
...
def build_email_page(page: ft.Page, lq: queue.Queue):
    ...
    txt_prefetch_workers = ft.TextField(
        label="Faturas baixadas ao mesmo tempo",
        hint_text="Recomendado: 5",
        value="5",
        width=280, height=44, border_radius=8, border_color=BORDER,
        focused_border_color=ORANGE, filled=True, fill_color="#F8FAFD",
        color=TEXT_MAIN,
    )
    txt_prefetch_window = ft.TextField(
        label="Faturas preparadas antecipadamente",
        hint_text="Fila de espera (Recomendado: 15)",
        value="15",
        width=280, height=44, border_radius=8, border_color=BORDER,
        focused_border_color=ORANGE, filled=True, fill_color="#F8FAFD",
        color=TEXT_MAIN,
    ...
    credencias_fatura_email = ft.Container(
        content=ft.Column([
            ...
            ft.Container(height=6),
            ft.Row([txt_prefetch_workers, ft.Container(width=12), txt_prefetch_window]),
            ft.Container(height=6),
            ft.Text(
            ...
            ft.Text(
                "Cuidado ao aumentar a quantidade de faturas baixadas ao mesmo tempo para evitar bloqueios no Waterfy.",
                size=11,
                color=ORANGE,
                italic=True,
            ),
    ...
    lbl_tpl_status = ft.Text(
        "Modelo: NOTIFICAÇÃO DE DEBITOS (-30 dias)",
        size=12,
        color=TEXT_MUTED,
    )
...
<body style="font-family:sans-serif;padding:15px;color:#333;">{body_html}</body></html>"""
    ...
    col_campos = ft.ResponsiveRow(columns=4, spacing=6, run_spacing=6)
    ...
    def refresh_col_buttons():
        ...
        if not state["columns"]:
            ...
            col_campos.controls.append(
                ft.Container(
                    col=4, content=ft.Text("Carregue um CSV primeiro.", size=11, color=TEXT_MUTED, italic=True)
                )
            )
        else:
                ...
                col_campos.controls.append(
                    ft.Container(
                        ...
                        content=ft.Row([
                            ...
                            ft.Text(col, size=11, color=WHITE, weight=ft.FontWeight.W_600, expand=True, overflow=ft.TextOverflow.ELLIPSIS),
                        ], spacing=0),
                        height=30,
                        bgcolor=NAVY,
                        border_radius=6,
    ...
    def iniciar_envio(e):
        ...
        try:
            ...
            prefetch_workers = int(txt_prefetch_workers.value.strip() or "5")
            prefetch_window = int(txt_prefetch_window.value.strip() or "15")
        except ValueError:
            _snack("Delay, lote ou opções de download inválidos!", ok=False); return
    ...
    def _registrar_fechamento_email():
        page._close_guard = {
            "kind": "email",
            "title": "Envio de e-mails em andamento",
            "message": (
                "Há um envio de e-mails em andamento.\n"
                "Se você sair agora, o sistema vai encerrar o lote atual, salvar os status na planilha "
                "e fechar quando isso terminar."
            ),
            "confirm_label": "Salvar e sair",
            "request_stop": lambda: parar_envio(None),
            "wait_message": "Salvando o status do envio de e-mails antes de fechar...",
        }

    def _limpar_fechamento_email():
        guard = getattr(page, "_close_guard", None)
        if isinstance(guard, dict) and guard.get("kind") == "email":
            page._close_guard = None
    ...
    def _executar_envio(
        ...
        def _thread():
            ...
            try:
                ...
                _OUTBOX_HIGH_WATERMARK = 30
                _OUTBOX_LOW_WATERMARK = 10
                _OUTBOX_POLL_SECONDS = 2
                _OUTBOX_DRAIN_TIMEOUT = 180
                # Configuração conservadora: poucos downloads paralelos para não
                # sobrecarregar o Waterfy nem misturar contexto de anexos.
                _PREFETCH_WINDOW = prefetch_window  # Quantidade de faturas seguintes preparadas antecipadamente (forma um "estoque" de PDFs prontos para envio imediato)
                _PREFETCH_WORKERS = prefetch_workers # Quantidade de downloads simultâneos no Waterfy em segundo plano (cuidado com valores altos para evitar bloqueio por excesso de acessos)

                prefetch_executor = concurrent.futures.ThreadPoolExecutor(max_workers=_PREFETCH_WORKERS)
                attachment_futures = {}
                prepared_attachments = {}

                def _row_ja_processada(row_dict):
                    status = str(row_dict.get("STATUS", "")).strip().upper()
                    return status.startswith("ENVIADO") or status == "PULADO MANUALMENTE"

                def _email_valido(row_dict):
                    dest = str(row_dict.get(dest_col, "")).strip()
                    return bool(dest and "@" in dest)

                def _cleanup_cached_attachment(path_anexo):
                    if not path_anexo or not os.path.isfile(path_anexo):
                        return
                    ...
                    if ap.startswith(_anexo_cache_abs + os.sep) or ap == _anexo_cache_abs:
                        try:
                            ...
                            pass
                        try:
                            current_dir = os.path.dirname(ap)
                            cache_root = _anexo_cache_abs.rstrip("\\/")
                            while current_dir and current_dir.startswith(cache_root + os.sep):
                ...
                def _outbox_count():
                    if not usar_outlook or outlook is None:
                        return 0
                    try:
                        namespace = outlook.GetNamespace("MAPI")
                        outbox_folder = namespace.GetDefaultFolder(4)  # olFolderOutbox
                        return int(outbox_folder.Items.Count)
                    except Exception:
                        return 0

                def _wait_for_outlook_capacity():
                    if not usar_outlook or outlook is None:
                        return
                    current_count = _outbox_count()
                    if current_count < _OUTBOX_HIGH_WATERMARK:
                        return

                    lq.put(
                        f"  ⏳ Caixa de saída do Outlook em {current_count} item(ns). "
                        f"Aguardando cair para {_OUTBOX_LOW_WATERMARK}...\n"
                    )
                    started_wait = time.time()
                    while not state["stop_flag"]:
                        current_count = _outbox_count()
                        if current_count <= _OUTBOX_LOW_WATERMARK:
                            lq.put(f"  ✓ Caixa de saída liberada ({current_count} item(ns)). Retomando disparo.\n")
                            return
                        if time.time() - started_wait >= _OUTBOX_DRAIN_TIMEOUT:
                            raise RuntimeError(
                                "A Caixa de Saída do Outlook não reduziu no tempo esperado. "
                                "Verifique se o Outlook está travado ou aguardando confirmação."
                            )
                        time.sleep(_OUTBOX_POLL_SECONDS)

                def _prepare_attachment(record):
                    if state["stop_flag"]:
                        return {"path_anexo": None, "aviso_anexo": "", "cancelled": True}
                ...
                def _schedule_prefetch(start_pos):
                    end_pos = min(len(rows_iter), start_pos + _PREFETCH_WINDOW)
                    for next_pos in range(start_pos, end_pos):
                        if next_pos in attachment_futures or next_pos in prepared_attachments:
                            continue
                        _, next_row_series = rows_iter[next_pos]
                        next_row = next_row_series.to_dict()
                        if _row_ja_processada(next_row) or not _email_valido(next_row):
                            continue
                        attachment_futures[next_pos] = prefetch_executor.submit(
                            _prepare_attachment,
                            {"pos": next_pos, "row": next_row},
                        )

                def _harvest_ready_attachments():
                    ready_positions = [future_pos for future_pos, future in attachment_futures.items() if future.done()]
                    for future_pos in ready_positions:
                        future = attachment_futures.pop(future_pos)
                        try:
                            prepared_attachments[future_pos] = future.result()
                        except Exception as exc:
                            prepared_attachments[future_pos] = {"error": exc}
                ...
                for pos, (idx, row_series) in enumerate(rows_iter):
                    ...
                    _schedule_prefetch(pos + 1)
                    _harvest_ready_attachments()
                    ...
                    path_anexo = None
                    try:
                        attachment_info = prepared_attachments.pop(pos, None)
                        if attachment_info is None:
                            future = attachment_futures.pop(pos, None)
                            if future is not None:
                                try:
                                    attachment_info = future.result()
                                except Exception as exc:
                                    attachment_info = {"error": exc}
                            else:
                                attachment_info = _prepare_attachment({"pos": pos, "row": row})
                        else:
                            attachment_futures.pop(pos, None)

                        if attachment_info.get("error") is not None:
                            raise attachment_info["error"]

                        path_anexo = attachment_info.get("path_anexo")
                        aviso_anexo = attachment_info.get("aviso_anexo", "")
                        if aviso_anexo:
                        ...
                        if usar_outlook:
                            ...
                            if path_anexo and os.path.isfile(path_anexo):
                                ...
                                _cleanup_cached_attachment(path_anexo)
                ...
                try:
                    if '_cleanup_cached_attachment' in locals():
                        for attachment_info in list(locals().get("prepared_attachments", {}).values()):
                            _cleanup_cached_attachment(attachment_info.get("path_anexo"))
                        for future in list(locals().get("attachment_futures", {}).values()):
                            if future.done():
                                try:
                                    attachment_info = future.result()
...
def main(page: ft.Page):
    ...
    topbar_title = ft.Text(
        "Itapoá Saneamento",
        size=13, color=WHITE,
        weight=ft.FontWeight.W_600,
    )

    dash_col, _ref_csvs, _ref_msgs, _ref_props, _ref_paradas = build_dashboard(
        num_operadores=_num_op, lq=lq, page=page, topbar_title_ref=topbar_title)
    ...
    def _humanize_better(linha: str) -> str | None:
        ...
        # Interceptar a stack trace GWT do Java quando o servidor do Waterfy sofre instabilidade/queda de banco
        if "UIException" in m and "JDBC Connection" in m:
            m = re.sub(
                r"ERRO\s*Erro ao processar\s+(\d{2}/\d{4}):.*",
                r"[FALHA] Servidor do Waterfy sobrecarregado ao tentar baixar \1 (Conexão com Banco de Dados perdida).",
                m,
                flags=re.I,
            )
            return m
    ...
    def _can(mod):
        return _role == "ADM" or _mods == "Todos" or mod in _mods_set
    ...
    NAV = [
        ...
        ("EMAIL",               "Disparo de E-mails", _can("Disparo de E-mails")),
        ("HELP",                "Ajuda",         True),
    ]
    ...
    def _open_app_dir(e):
        try:
            if hasattr(os, "startfile"):
                os.startfile(_app_base_dir())
            else:
                subprocess.Popen(["xdg-open", _app_base_dir()])
        except Exception:
            pass
        ...
        ft.Container(
            ...
            content=ft.Row([
                ...
                ft.IconButton(
                    icon=ic("FOLDER_OPEN"),
                    icon_size=18,
                    icon_color=ft.Colors.with_opacity(0.88, WHITE),
                    tooltip="Abrir local do arquivo",
                    on_click=_open_app_dir,
                ),
                ft.Container(width=4),
                ft.Container(
                    content=ft.Row([
    ...
    _ui_refs["topbar_user"] = topbar.controls[1].content.controls[4].content.controls[1]
    _ui_refs["clock_chip"] = topbar.controls[1].content.controls[6]
    ...
    def _on_window_event(e):
        ...
        def _salvar_e_sair(ev):
            ...
            page._pending_close_after_tasks = True
            request_stop = guard.get("request_stop")
            if callable(request_stop):
                try:
                    ...
                    _destroy()
                    return
            wait_message = str(guard.get("wait_message", "") or "").strip()
            if wait_message:
                try:
                    page.snack_bar = ft.SnackBar(
                        ft.Text(wait_message, color=WHITE),
                        bgcolor=ORANGE,
                        open=True,
                    )
                    page.update()
        ...
        dlg = ft.AlertDialog(
            ...
            title=ft.Row([
                ...
                ft.Container(width=8),
                ft.Text(str(guard.get("title", "Processo em andamento")), size=15,
                        weight=ft.FontWeight.BOLD, color=TEXT_MAIN),
            ], spacing=0),
            content=ft.Text(
                str(guard.get("message", "Há um processo em andamento.\nDeseja salvar o progresso antes de sair?")),
                size=13, color=TEXT_MAIN,
            ),
            actions=[
                ft.TextButton(
                ...
                ft.Container(
                    content=ft.Text(str(guard.get("confirm_label", "Salvar e sair")), size=13,
                                    weight=ft.FontWeight.BOLD, color=WHITE),
                    bgcolor=ORANGE, border_radius=8,
