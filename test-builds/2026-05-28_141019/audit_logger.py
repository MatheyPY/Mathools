...
class AuditLogger:
    ...
    def log_email_dispatch(
        ...
        """
        Log de disparo de email de comunicado LGPD.
        
        Rastreabilidade completa de envios:
        - Horário automático (timestamp do log)
        - Usuário responsável (matrícula/login)
        - Destinatário (email)
        - Matrícula do cliente
        - Cidade/região da ação
        - Faturas em aberto no momento do envio
        - Status do envio
        
        Conformidade:
        - LGPD Art. 37 (Rastreabilidade de processamento)
        - Lei 5.172/1966 (Retenção 5 anos para cobrança)
        - Resolução de disputas judiciais
        
        Args:
            user: Identificador do usuário (matrícula/login) que disparou o envio
            recipient_email: Email do destinatário do comunicado
            matricula: Matrícula do cliente/imóvel
            open_invoices: Lista com números das faturas em aberto no momento do envio
            email_subject: Assunto do email enviado (sem dados pessoais)
            city: Cidade/região da ação (para rastreabilidade geográfica)
            success: True se enviado com sucesso, False se falhou
            error_detail: Detalhe do erro (se houver)
        """
        ...
        # Formatar lista de faturas
        open_invoices = open_invoices or []
        invoices_str = ", ".join(str(f) for f in open_invoices) if open_invoices else "Nenhuma"
        
        # Construir mensagem de auditoria com as informações
        message = f"Comunicado enviado | Matrícula: {matricula} | Cidade: {city} | Email: {recipient_email} | " \
                  f"Faturas em aberto: [{invoices_str}]"
        
        if email_subject:
            message += f" | Assunto: {email_subject}"
    ...
    def log_batch_email_dispatch(
        ...
        """
        Log de conclusão de um lote de disparos de email.
        
        Resumo agregado para análise de campanhas.
        
        Args:
            user: Identificador do usuário que disparou o lote
            total_recipients: Total de destinatários no lote
            successful_sends: Quantidade de envios bem-sucedidos
            failed_sends: Quantidade de envios que falharam
            process_duration_seconds: Duração total do processo
            detail: Detalhes adicionais (nome da campanha, etc)
        """
        
        extra = {
            'user': user,
            'action': 'EMAIL_DISPATCH_BATCH',
            'status': 'SUCCESS' if failed_sends == 0 else 'PARTIAL',
        }
        
        message = f"Lote de comunicados concluído | Total: {total_recipients} | " \
                  f"Sucesso: {successful_sends} | Falhas: {failed_sends}"
        
        if process_duration_seconds is not None:
            message += f" | Duração: {process_duration_seconds:.2f}s"
