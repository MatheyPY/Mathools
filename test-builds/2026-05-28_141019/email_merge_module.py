...
BUILTIN_EMAIL_TEMPLATES: List[Tuple[str, str, str]] = [
    ...
    ("campanha_desenrola", "CAMPANHA ESPECIAL DE NEGOCIAÇÃO (DESENROLA BRASIL)", "campanha_desenrola_brasil.html"),
]
...
class MergeEngine:
    ...
    def _resolve_company_info(self, row_up: Dict[str, str]) -> Dict[str, str]:
        """Resolve dados dinâmicos da empresa baseado nas colunas de cidade do plano de fundo.

        Aceita variações de nomes de coluna (com acentos, espaços, etc.), ex: 'MUNICÍPIO',
        'Cidade Imóvel', 'CIDADE_IMOVEL' e similares.
        """
        ...
        # Normaliza as chaves da linha para procurar a coluna de cidade independentemente
        # de acentos e formatação.
        norm_row = { _norm_key(k): v for k, v in row_up.items() }

        cidade_row = ""
        for candidate in ("CIDADE_IMOVEL", "CIDADE", "MUNICIPIO"):
            if candidate in norm_row and str(norm_row[candidate]).strip():
                cidade_row = str(norm_row[candidate])
                break
        # ultima tentativa: procura qualquer chave que contenha 'CIDADE' ou 'MUNICIPIO'
        if not cidade_row:
            for k, v in norm_row.items():
                if ("CIDADE" in k or "MUNICIPIO" in k) and str(v).strip():
                    cidade_row = str(v)
                    break
